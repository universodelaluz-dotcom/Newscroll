...
- **Curación a demanda**: `POST /api/curate` extrae la página y devuelve un bloque `{ curated: {...} }`. Usa el formulario de la barra para pegar cualquier URL (puedes meter múltiples webs); cada curación se añade a la parte superior del scroll y aparece también en el panel inferior con urgencia y etiquetas, de modo que el ticker mezcla esos titulares procesados con el feed RSS real.
